...
where U.id = UP.user_id  and CTU.id_tipo_usuario = UP.id_tipo_usuario and CEA.id_estatus = UP.id_estatus and UP.id_tipo_usuario = 2 order by (U.id) limit 1000");
	...
	{
        $query = "select 
                        U.id, U.username,U.recovery,
                        (select group_concat(distinct directo) 
                            from afiliar where id_afiliado = U.id) sponsor,
                        U.email,UP.nombre, UP.apellido,
                        CTU.descripcion ,CEA.descripcion estatus , 
                         (select distinct group_concat(tr.nombre)
                            from tipo_red tr 
                            where tr.id in (select af.id_red 
                                                from afiliar af 
                                                where af.id_afiliado = U.id)
                            ) redes
                    from 
                          users U, 
                          user_profiles UP,
	...
	function get_tabla_por_nombre_buscado($nombre_buscado, $id_red)
	{
        $query = "select U.id,
                           ...
                           (select distinct group_concat(tr.nombre)
                            ...
                            where tr.id in (select af.id_red 
                                              from afiliar af 
                                              where af.id_afiliado = U.id)) redes
                    from users U,
                         user_profiles UP,
                         cat_tipo_usuario CTU,
                         cat_estatus_afiliado CEA
                    where U.id = UP.user_id
                      ...
                      and CEA.id_estatus = UP.id_estatus
                      and UP.id_tipo_usuario = 2
                      and upper(UP.nombre) like upper('%$nombre_buscado%')
                    group by (U.id)
                    order by (U.id);";
        $q=$this->db->query($query);
		return  $q->result();
