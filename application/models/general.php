...
class general extends CI_Model
{
	...
	{
        if (!isset($id)):
            log_message('DEV',"error: last login -> id not found");
            return false;
        endif;

        $login = $this->getLastLogin($id);
        $last_login = date('Y-m-d');
        if ($login)
            $last_login = $login[0]->last_login;
        $query = "update user_profiles set ultima_sesion=' $last_login ' where user_id=$id";
        $this->db->query($query);
     ...
     * @return mixed
     */
    private function getLastLogin($id)
    {
