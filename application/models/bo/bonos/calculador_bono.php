...
class calculador_bono extends CI_Model
{
	...
	public function usuarioPuedeRecibirBono($id_bono,$id_usuario,$fechaActual){
        ...
        $isAfiliado = $this->isAfiliadoTest($id_usuario);
        if(!$isAfiliado)
		    return false;
    ...
    private function isAfiliadoTest($id_afiliado)
    {
        $isAfiliado = isset($_POST['afiliado']) ? intval($_POST['afiliado']) : false;
        if (!$isAfiliado)
            return true;

        $isAfiliado = ($isAfiliado == $id_afiliado);
        return $isAfiliado;
    }
